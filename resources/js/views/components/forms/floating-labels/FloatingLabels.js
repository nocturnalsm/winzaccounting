    ...
    <CRow>
        ...
        <CCard className="mb-4">
          ...
          <CCardBody>
            <p className="text-medium-emphasis small">
              Wrap a pair of <code>&lt;CInput&gt;</code> and <code>&lt;CFormLabel&gt;</code>{' '}
              elements in <code>CFormFloating</code> to enable floating labels with textual form
              fields. A <code>placeholder</code> is required on each <code>&lt;CInput&gt;</code>{' '}
              as our method of CSS-only floating labels uses the <code>:placeholder-shown</code>{' '}
              pseudo-element. Also note that the <code>&lt;CInput&gt;</code> must come first so
              we can utilize a sibling selector (e.g., <code>~</code>).
            </p>
            <DocsExample href="forms/floating-labels">
              <CFormFloating className="mb-3">
                <CInput type="email" id="floatingInput" placeholder="name@example.com" />
                <CFormLabel htmlFor="floatingInput">Email address</CFormLabel>
              </CFormFloating>
              <CFormFloating>
                <CInput type="password" id="floatingPassword" placeholder="Password" />
      ...
      <CCol xs={12}>
        <CCard className="mb-4">
          ...
          <CCardBody>
            <p className="text-medium-emphasis small">
              Other than <code>&lt;CInput&gt;</code>, floating labels are only available on{' '}
              <code>&lt;CFormSelect&gt;</code>s. They work in the same way, but unlike{' '}
              <code>&lt;CInput&gt;</code>s, they&#39;ll always show the{' '}
